...
class SignalGenerator:
    ...
    def generate(self, df: pd.DataFrame) -> Signal:
        ...
        Args:
            df: DataFrame with OHLCV + all indicator columns (from IndicatorEngine)

        Returns:
            Signal dataclass
        """
        if df.empty or len(df) < 30:
            logger.warning("⚠️  Not enough data for signal generation")
        ...
        if p_macd <= p_sig and macd > macd_sig:          # bullish crossover
            score += 25
            reasons.append("✅ MACD bullish crossover → BUY signal")
        elif p_macd >= p_sig and macd < macd_sig:        # bearish crossover
            score -= 25
        ...
        if close > vwap:
            score += 15
            reasons.append(f"✅ Price (₹{close}) > VWAP (₹{vwap:.2f}) → Bullish bias")
        else:
            score -= 15
            reasons.append(f"🔴 Price (₹{close}) < VWAP (₹{vwap:.2f}) → Bearish bias")
        ...
        bb_pct = (close - bb_lower) / (bb_upper - bb_lower + 1e-9)  # 0=lower, 1=upper

        if bb_pct < 0.20:
            score += 20
            reasons.append(f"✅ Price near BB Lower band ({bb_pct:.0%}) → Reversal likely (BUY)")
        elif bb_pct > 0.80:
            score -= 20
            reasons.append(f"🔴 Price near BB Upper band ({bb_pct:.0%}) → Reversal likely (SELL)")
        else:
            reasons.append(f"➡️  Price mid-Bollinger Band ({bb_pct:.0%})")
